<script lang="ts">
    ...
    // ==================== ACCIONES ====================
    function abrirNuevo() {
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.profesores.crear}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
        mostrarNuevo = true;
    ...
    function abrirEditar(p: Profesor, e: Event) {
        ...
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para ${MENSAJES_PERMISOS.profesores.editar}. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
    ...
    function abrirImportar() {
        ...
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para importar profesores. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
    ...
    async function exportarCSV() {
        if (!puedeGestionarProfesores()) {
            const rol = authStore.user?.rol || "Usuario";
            toast.error(
                `No tienes permisos para exportar profesores. Solo Director y Admin pueden realizar esta acción. Tu rol actual: ${rol}`,
            );
            return;
        }
